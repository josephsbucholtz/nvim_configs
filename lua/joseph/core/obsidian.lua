...
local function translate_obsidian_datetime_format(format)
	local tokens = {
		...
		{ "ss", "%S" },
		{ "A", "%p" },
		{ "a", "__JOSEPH_OBSIDIAN_LOWER_AMPM__" },
	}
...
local function render_obsidian_datetime(format)
	...
	return rendered:gsub("__JOSEPH_OBSIDIAN_LOWER_AMPM__", function()
		return os.date("%p"):lower()
	end)
end

function M.patch_template_substitutions()
	if vim.g.joseph_obsidian_template_patch_applied then
		return
	end
	...
	vim.g.joseph_obsidian_template_patch_applied = true
end
